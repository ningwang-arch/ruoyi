...
//           variables.put(ProcessConstants.FLOWABLE_SKIP_EXPRESSION_ENABLED, true);
            ...
            ProcessInstance processInstance = bizRuntimeService.startProcessInstanceById("task_abc", procDefId, variables);
...
//                taskService.setAssignee(task.getId(), sysUser.getUserId().toString());
    ...
    @Override
    public AjaxResult startProcessInstanceWithKeyById(String procDefId, String businessKey, Map<String, Object> variables) {
        try {
            ProcessDefinition processDefinition = repositoryService.createProcessDefinitionQuery().processDefinitionId(procDefId)
                    .latestVersion().singleResult();
            if (Objects.nonNull(processDefinition) && processDefinition.isSuspended()) {
