...
public class FlowTaskServiceImpl extends FlowServiceFactory implements IFlowTaskService {
    ...
    @Override
    public AjaxResult complete(BizFlowTaskVo taskVo) {
     ...
     */
    @Override
    public void taskReject(BizFlowTaskVo flowTaskVo) {
    ...
    @Transactional(rollbackFor = Exception.class)
    @Override
    public void taskReturn(BizFlowTaskVo flowTaskVo) {
        if (taskService.createTaskQuery().taskId(flowTaskVo.getTaskId()).singleResult().isSuspended()) {
            throw new CustomException("任务处于挂起状态");
        }
        // 当前任务 task
    ...
    @Override
    public AjaxResult bizMyProcess(Integer pageNum, Integer pageSize) {
...
//            }
    ...
    @Override
    public AjaxResult bizTodoList(Integer pageNum, Integer pageSize) {
        Page<BizFlowTaskDto> page = new Page<>();
        Long userId = SecurityUtils.getLoginUser().getUser().getUserId();
        TaskQuery taskQuery = taskService.createTaskQuery()
                .active()
...
//            SysUser startUser = sysUserService.selectUserById(Long.parseLong(task.getAssignee()));
    ...
    @Override
    public AjaxResult bizFinishedList(Integer pageNum, Integer pageSize) {
        Page<BizFlowTaskDto> page = new Page<>();
        Long userId = SecurityUtils.getLoginUser().getUser().getUserId();
        HistoricTaskInstanceQuery taskInstanceQuery = historyService.createHistoricTaskInstanceQuery()
                .includeProcessVariables()
                ...
                .desc();
        List<HistoricTaskInstance> historicTaskInstanceList = taskInstanceQuery.listPage(pageSize * (pageNum - 1), pageSize);
        List<BizFlowTaskDto> hisTaskList = Lists.newArrayList();
        for (HistoricTaskInstance histTask : historicTaskInstanceList) {
            BizFlowTaskDto flowTask = new BizFlowTaskDto();
            // 当前流程信息
            flowTask.setTaskId(histTask.getId());
...
//        result.put("finished",true);
    ...
    private TugFee hook(Task task, BizFlowTaskVo taskVo) {
    ...
    private Task getNextTask(Task task) {
        List<FlowNode> flowNodes = getFlowNodes(task.getProcessDefinitionId());
        for (FlowNode flowNode : flowNodes) {
            if (flowNode.getId().equals(task.getTaskDefinitionKey())) { // 当前节点
                int index = flowNodes.indexOf(flowNode); // 当前节点的索引
                if (index < flowNodes.size() - 1) { // 不是最后一个节点
                    FlowNode nextFlowNode = flowNodes.get(index + 1); // 下一个节点
                    Task nextTask = taskService.createTaskQuery()
                            .processInstanceId(task.getProcessInstanceId())
                            .taskDefinitionKey(nextFlowNode.getId())
                            .singleResult();
                    return nextTask;
                }
            }
    ...
    private List<FlowNode> getFlowNodes(String processDefinitionId) {
        // 获取流程定义
        BpmnModel bpmnModel = repositoryService.getBpmnModel(processDefinitionId);
        // 获取流程定义的所有节点
